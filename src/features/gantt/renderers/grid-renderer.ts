import { format, startOfDay, startOfHour, addHours, addDays } from "date-fns";
import type { ViewportState, VirtualRow, ZoomLevel } from "../types";
import { clearCanvas, drawDashedLine } from "../utils/canvas-utils";
import { ZOOM_CONFIGS } from "../store/ui-store";
...
export class GridRenderer {
	...
	private renderTimeGrid(
		...
		const visibleEndTime = visibleStartTime + (this.width / pixelsPerHour) * 3600000;
		...
		for (
			...
			const isMajor = hourOfDay % (intervals.major / (intervals.minor || 1)) === 0;
