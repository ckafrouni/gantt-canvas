import type { ViewportState, ZoomLevel } from "../types";
...
export class ViewportManager {
	...
	timeToX(timestamp: number): number {
		const hoursFromOrigin = (timestamp - this.viewport.timeOrigin) / MS_PER_HOUR;
		return hoursFromOrigin * this.viewport.pixelsPerHour - this.viewport.scrollX;
	}
	 ...
	 */
	viewportToData(x: number, y: number): { timestamp: number; virtualY: number } {
		return {
...
// Factory function
export function createViewportManager(viewport: ViewportState): ViewportManager {
