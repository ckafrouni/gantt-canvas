...
export function useGanttInteractions(
	...
	const hitTest = useCallback(
		(clientX: number, clientY: number): { taskId: TaskId | null; zone: HitZone } => {
			...
			// Query spatial index
			const hitTaskIds = indexManager.queryPoint(dataCoords.timestamp, dataCoords.virtualY);
			...
			const taskWidth = viewportManager.durationToWidth(task._endTime - task.startTime);
