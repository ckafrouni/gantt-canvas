...
import type { DragState, Task, ViewportState } from "../types";
import {
	clearCanvas,
	drawDashedLine,
	fillRoundRect,
...
export class InteractionRenderer {
	...
	private renderDragPreview(
		...
		if (dragState.type === "reassign" || dragState.type === "move") {
			...
			ctx.fillStyle = hasCollisions
				? COLORS.dropZoneInvalid
				: COLORS.dropZoneValid;
	...
	private renderSnapLines(
		...
		const snappedEnd =
			Math.round(dragState.previewEndTime / snapInterval) * snapInterval;
	...
	private renderTimeTooltip(
		...
		// Tooltip position
		const tooltipX =
			dragState.type === "resize-end" ? taskX + taskWidth : taskX;
