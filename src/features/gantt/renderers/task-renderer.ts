...
import type { Task, TaskId, ViewportState, VirtualRow } from "../types";
import {
	clearCanvas,
	drawTruncatedText,
	fillRoundRect,
...
export class TaskRenderer {
	...
	render(
		...
		const visibleStartTime =
			viewport.timeOrigin +
			(viewport.scrollX / viewport.pixelsPerHour) * 3600000;
	...
	private renderTask(
		...
		const width = this.durationToWidth(
		...
		for (let i = 0; i < phases.length; i++) {
			...
			if (isFirst && isLast) {
				...
				ctx.arcTo(
					currentX + phaseWidth,
					y + height,
					currentX + phaseWidth - TASK_BORDER_RADIUS,
					y + height,
					TASK_BORDER_RADIUS,
				);
